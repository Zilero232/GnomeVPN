...
**Windows has no equivalent and must not fake one.** `taskkill` without `/F`
posts WM_CLOSE to a top-level window, and sing-box is spawned by a service with
no desktop and no window — the request goes nowhere while `taskkill` still
reports success, so every disconnect burned the full `SHUTDOWN_GRACE` before the
kill. That was tried and reverted; it made disconnects visibly slower and fixed
nothing.
