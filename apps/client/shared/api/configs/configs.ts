import type { ConfigStatus, DownloadedConfig, IssueConfigRequest } from '@gnomevpn/schemas';
...
export const listConfigStatus = async (): Promise<ConfigStatus> => {
  const { data } = await api.get('/configs/status');
