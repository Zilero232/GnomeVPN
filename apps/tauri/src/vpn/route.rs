...
pub fn windows_delete_args(destination: &str, mask: &str) -> Vec<String> {
    vec!["delete".into(), destination.into(), "MASK".into(), mask.into()]
}
...
mod tests {
    ...
    #[test]
    fn delete_never_targets_the_system_default_route() {
        let host = windows_delete_args("203.0.113.1", "255.255.255.255");
        let lower = windows_delete_args("0.0.0.0", "128.0.0.0");
        let upper = windows_delete_args("128.0.0.0", "128.0.0.0");

        for args in [&host, &lower, &upper] {
            let joined = args.join(" ");

            assert!(
                !joined.contains("0.0.0.0 MASK 0.0.0.0"),
                "deleting 0.0.0.0/0 would wipe the physical default route: {joined}"
            );
        }
    }

    #[test]
    fn half_routes_cover_the_whole_ipv4_space_without_touching_default() {
        for (destination, mask) in HALF_ROUTES {
            assert_eq!(mask, "128.0.0.0", "half routes must use a /1 mask");
            assert!(destination == "0.0.0.0" || destination == "128.0.0.0");
        }
    }

    #[test]
    fn add_args_bind_the_route_to_an_interface() {
        let args = windows_add_args("0.0.0.0", "128.0.0.0", "10.8.0.1", "42", "1");
        let joined = args.join(" ");

        assert!(joined.contains("IF 42"), "route must be pinned to the tunnel interface");
        assert!(joined.contains("10.8.0.1"), "gateway must be the tunnel peer");
    }
