...
└── mobile_vpn/      # Android only — owns the tunnel itself
    ...
    ├── engine.rs    # spawns xray, feeds the TUN fd to tun2proxy
...
└── libs/            # xray as libxray.so, per ABI
...
the xray config and `tun2proxy` — is shared; `build_xray_config` lives in
...
Kotlin sources, `libxray.so` per ABI, and the manifest entries for `VpnService` —
...
why xray ships as `libxray.so` in `jniLibs` rather than as an asset, and why the
path comes from `ApplicationInfo.nativeLibraryDir` rather than being assembled by
hand: the install location differs on a work profile or an SD card.
...
the native binaries (`wintun.dll`, `xray.exe`, the service), the tray icons, the
...
- `sync-bin.mjs` — copies `wintun.dll` and `xray.exe` into `target/{debug,release}`. Both are loaded from the service's own directory, so a dev run cannot find them otherwise.
