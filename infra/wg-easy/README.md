...
- Открытый наружу `51820/udp`. Порт `51821/tcp` (REST/панель) **не должен** торчать в интернет —
  в compose он привязан к `127.0.0.1`; backend ходит на него через приватную сеть или SSH-туннель.
...
- `wgEasyUrl` — адрес REST API, например `http://10.0.0.5:51821` (приватная сеть), не публичный URL.
